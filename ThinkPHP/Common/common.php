...
// 字段过滤 处理函数1：对content字段写入前进行处理
function contentWriteFilter($var){
    $url = "/(https?|ftps?):\/\/(www|mail|news)\.([^\.\/]+)\.(com|org|net)/i";
    return preg_replace($url, '<a href="\1://\2.\3.\4">${1}://${2}.${3}.${4}</a>', $var);
}

// 字段过滤 处理函数2：读取content字段前进行处理
function contentReadFilter($var){
    ...
    // UBB替换 
    return preg_replace($ubbcodes, $htmls, $var);
}